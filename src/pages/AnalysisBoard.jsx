...
const API_BASE_URL = "https://sanchez-eight.vercel.app";
...
function AnalysisBoard({ onDatabaseUpdate }) {
  ...
  useEffect(() => {
    const getMovesForPosition = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/api/get-move?fen=${fen}`);
  ...
  async function handleSubmitPgn(event) {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/add-pgn`, {
        method: "POST",
  ...
  async function handleResetDatabase() {
    if (
      ...
      try {
        await fetch(`${API_BASE_URL}/api/reset-database`, {
          method: "POST",
        });
